...
JAROCK_PROXY_TIMEOUT  = int(os.getenv('JAROCK_PROXY_TIMEOUT', 70))   # ScraperAPI retries server-side for up to ~60s
JAROCK_BACKOFF_BASE   = float(os.getenv('JAROCK_BACKOFF_BASE', 3.0))  # seconds, doubles each retry

# --- ScraperAPI config ---
SCRAPERAPI_API_KEY   = os.getenv('SCRAPERAPI_API_KEY', '')
SCRAPERAPI_ENDPOINT  = 'https://api.scraperapi.com'
SCRAPERAPI_RENDER    = os.getenv('SCRAPERAPI_RENDER', 'false')   # 'true' enables JS rendering (costs more credits)
SCRAPERAPI_PREMIUM   = os.getenv('SCRAPERAPI_PREMIUM', 'false')  # 'true' uses premium/residential proxies (costs more credits)
SCRAPERAPI_COUNTRY   = os.getenv('SCRAPERAPI_COUNTRY', '')       # e.g. 'us' — leave blank to let ScraperAPI choose
log.info(f"SCRAPERAPI_API_KEY loaded: {'YES' if SCRAPERAPI_API_KEY else 'NO'}")
...
def _get(url):
    ...
    timeout, since ScraperAPI itself can take a while to come back on tougher
    ...
    # ScraperAPI isn't a proxy you route requests through — it's a GET endpoint
    # that fetches the target URL server-side (handling proxies/headless browser/
    # CAPTCHAs for you) and hands back the raw HTML.
    if not SCRAPERAPI_API_KEY:
        log.error(f'No SCRAPERAPI_API_KEY set; cannot bypass block for {url!r}')
        return None
    ...
    if SCRAPERAPI_PREMIUM.lower() == 'true':
        sa_params['premium'] = 'true'
    if SCRAPERAPI_COUNTRY:
        sa_params['country_code'] = SCRAPERAPI_COUNTRY

    for attempt in range(1, JAROCK_PROXY_RETRIES + 1):
        try:
            r = requests.get(SCRAPERAPI_ENDPOINT, params=sa_params, timeout=JAROCK_PROXY_TIMEOUT)
            # ScraperAPI returns 500 if it couldn't fetch the page within its own
            # retry window (it doesn't bill you for that failed attempt), so it's
            # safe to retry. 401/403 mean a bad/blocked key — retrying won't help.
            if r.status_code == 500:
                log.warning(f'ScraperAPI returned 500 (unfetchable) for {url!r} — retrying')
                if attempt < JAROCK_PROXY_RETRIES:
                    time.sleep(JAROCK_BACKOFF_BASE * attempt)
                continue
            if r.status_code in (401, 403):
                log.error(f'ScraperAPI auth/permission error ({r.status_code}) for {url!r} — check API key/plan')
                return None
            r.raise_for_status()
